...
class Balances extends CI_Controller {
	...
	public function addsignup(){
        ...
        $email=$this->input->post('email');
	    $num=$this->Balance->checkEmail($email,$password);
	    if($num>0)
	    {
	     $this->session->set_flashdata('email', 'email_id is already existing');
	     redirect('Balances/signup');
	    }
	    else{
        $this->Balance->addSignup($data);
	    $this->session->set_flashdata('registor', 'successfully registrated');
	    redirect('Balances/signup');
	    }	
	}
	...
	public function addlogin(){
		...
		$num_row=$this->Balance->islogin($email,$password);
		if($num_row>0){
		    redirect('Balances/balance');
		}
	
	   else{
	   $this->session->set_flashdata('email', 'invalid email and password');
	   redirect('Balances/login');	
	   }
	}
    ...
    $filename = 'expensivedeatile_'.date('Ymd').'.csv'; 
   ...
   // get data 
   $cur_date=date("m");
   $expensiveData =$this->Balance->export_expensive($cur_date);

   // file creation 
    $file = fopen('php://output', 'w');
    $header = array("Amount","Category","Note","Date"); 
    fputcsv($file, $header);
    foreach ($expensiveData  as $key=>$line){ 
     fputcsv($file,$line); 
    }
    fclose($file); 
    exit; 
   ...
   // file name 
   $filename = 'incomedeatile_'.date('Ymd').'.csv'; 
   header("Content-Description: File Transfer"); 
   header("Content-Disposition: attachment; filename=$filename"); 
   header("Content-Type: application/csv; ");
   ...
   // get data 
   $cur_date=date("m");
   $incomeData =$this->Balance->exportIncome($cur_date);

   // file creation 
   $file = fopen('php://output', 'w');
   $header = array("Amount","Category","Note","Date"); 
   fputcsv($file, $header);
   foreach ($incomeData  as $key=>$line){ 
     fputcsv($file,$line); 
   }
   fclose($file); 
   exit; 
