...
class Balance extends CI_Model {
   ...
   function getAllIncomeCategory(){
	...
	function month_income_transaction($cur_date){
		$this->db->select('sum(income.amount) as amount,income_category.name,income.note');
		$this->db->from('income');
		$this->db->join('income_category','income_category.id=income.category');
		$this->db->where('month(cur_date)',$cur_date);
		$this->db->group_by('income.category');
		$query=$this->db->get();
	    return $query->result_array();
	...
	function month_expensive_transaction($cur_date){
		$this->db->select('sum(expense.amount) as amount,expensive_category.name,expense.note');
		$this->db->from('expense');
		$this->db->join('expensive_category','expensive_category.id=expense.category');
		$this->db->where('month(cur_date)',$cur_date);
		$this->db->group_by('expense.category');
		$query=$this->db->get();
	    return $query->result_array();
	...
	function export_expensive($cur_date){
	    $this->db->select('expense.amount,expensive_category.name,expense.note,expense.cur_date');
		$this->db->from('expense');
		$this->db->join('expensive_category','expensive_category.id=expense.category');
		$this->db->where('month(cur_date)',$cur_date);
		$this->db->group_by('expense.category');
	...
	function exportIncome($cur_date){
		$this->db->select('income.amount,income_category.name,income.note,income.cur_date');
		$this->db->from('income');
		$this->db->join('income_category','income_category.id=income.category');
		$this->db->where('month(cur_date)',$cur_date);
		$this->db->group_by('income.category');
